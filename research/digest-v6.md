...
**SUPERSEDES `digest-v5.md`, `digest-v4.md`, `digest-v3.md`, `digest-v2.md`, and `digest.md`.** Observed and retrieved 2026-09-11. Git history is the archive. This generation refreshes every surviving field and supplies the checked basis for the nine profiles shipped by Track 5.
...
| `anthropic/claude-fable-5-1` | minimal, low, medium, high, xhigh, max | low–max | minimal | 1,000,000 / 128,000 | tier 4, unsourced Track 5 judgment |
| `google-vertex/gemini-3.8-flash` | low, medium, high | all three | none | 1,048,576 / 65,536 | tier 2, unsourced |
| `openai/gpt-6-astra` | low, medium, high, xhigh, max | all five | none | 1,050,000 / 128,000 | tier 4, unsourced |
...
- Fable 5.1: DeepSWE has no row and supplies no coding default. Low through max are measured through exact-effort AA results with fallback enabled. Refuse zero-retention work unless express model-specific authorization and required provider and account configuration are confirmed. Do not reuse predecessor cache probes. `[A4, A6, A9, A10]`
- Gemini: medium is the DeepSWE coding setting selected by the interval rule. Keep both published Terminal-Bench figures and the unresolved deployment difference. Vertex measurements do not establish alias-route cache, privacy, adapter, or effort behavior. `[G]`
- Astra: medium is the DeepSWE coding setting selected by the interval rule. Keep Standard and Provider Adapter ARC-AGI-3 results separate. The adapter is not a normal Slate route. `[G]`
...
These are model-and-effort statements. Track 5 renders the approved benchmark summaries as guidance. It derives no automatic task classifier or route.
...
## Gemini and Astra source basis

| spec | aliases | ladder | rejected requested controls | measured | context / output | cache | tier status |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
Pinned pi 0.83 supports both through the remote-catalogue overlay and generic adapters. No SDK upgrade is required. `apiRejectedLevels` protects the requested control before pi can omit or silently substitute it. Track 5 production profiles and named-model tests use this final field comparison. `[G]`
