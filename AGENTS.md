...
- `verification/logical-model-import-check.ts` and the `logical-*` checks — the shared TypeScript-parser control that keeps the three disconnected logical-model modules unreachable from recursively scanned `.ts` and `.mjs` runtime sources. It fails on forbidden literal references, syntax errors and new unreviewed computed module references. The only reviewed computed exception is the existing writing-checker import. The wrapper refuses with exit 2 before checks when the exact-pinned TypeScript compiler is unavailable.
...
- **Re-run it after any change to a covered module or source path.** Covered production modules are:
  ...
  - `verification/logical-model-import-check.ts`, which also requires the focused logical-model policy tests.
