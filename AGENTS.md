...
The shipped doctrine uses SMALL, MEDIUM and LARGE size grades. Eight risk-defined focus areas select additional gates through judged proofs. Reviewer I supplies one general review floor. Draft pull request publishing is optional.
...
- the shipped workflow contracts. These pin complete safety-floor and focus-table content, the eight risk definitions, and the marked test-quality definition. They also cover the eight-condition SMALL fast path, Reviewer I rows, the non-local logic defect charter and production-cap class, split prose and licensing charters, lifecycle ownership, retired-role and prefix absence, and unique named headings across all five workflow documents.
