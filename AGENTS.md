...
Dispatch also carries **action-level model routing**: `router.models` in the project's `slate.json` names a CLOSED candidate list, resolved once per session, against which each action's explicit model and effort are guarded. Configured survivor order is preserved. It is off by default: an empty list adds no candidate-list policy or routing table. Context-size substitution and long-context billing notices are not part of action routing in either router state. Per-action arguments and pre-existing live failover holds remain active. Reference: `docs/model-routing.md`, which the doctrine cites at runtime alongside the workflow/review/design docs.
...
- `extension/mode.ts`'s **action-routing doctrine rule** (`doctrine-*`, driven through `registerSlateMode`'s `before_agent_start` handler with a fabricated resolution): that a trusted router-OFF session gets only the explicit dispatch vocabulary and session base model, that an UNTRUSTED project gets no routing rule even with `router.models` fully configured (SE3's trust re-gate — defence in depth, so its removal has no visible symptom), that the conditional tail rules are numbered by position, its **injection safety** — the rule deliberately bypasses `sanitizeForDoctrine` (that sanitizer strips `|`, which would destroy the table), so the narrow `cell()` is the entire defence and the checks attack it structurally — that no research trace tag or `nonPreferred` reason leaks into the prompt, and a **size budget**, because this text is injected into every session's system prompt and paid for on every turn. The group is voided by `profiles-load`: one of its checks renders the real shipped table;
- the shipped workflow contracts. These pin complete safety-floor and focus-table content, the eleven risk definitions, and the marked test-quality definition. They also cover focus states, per-track design entry, approved area removal, marker availability, conditional Reviewer I composition and separation, no-area model choice, documentation-only classification, all four production defect charters and production-cap classes, split prose and licensing charters, lifecycle ownership, and unique named headings across all five workflow documents. Four complete dispatch-policy units protect the implementation-reference obligation, research-log lifecycle, reviewer-input contract and intact implementer-episode exception. Each unit resolves exactly once and equals an independent expectation. Counterfactuals must reject policy weakening and contradictory additions inside a unit. Benign changes outside the units remain accepted. The bounded reference and charter measurements stay protected.
...
- `extension/model-router.ts` — the model router: the `router` config sanitizer, candidate resolution (validation drops, configured-order preservation, registry base rates, W1/W3/failover-coverage warnings, de-duplication and memoization) and the dispatch-side effort predicate;
- `extension/logical-model-recovery.ts`, `extension/logical-model-adapters.ts`, `verification/logical-model-import-check.ts` and the `logical-*` checks — the disconnected recovery planners and injected execution adapters plus the shared TypeScript-parser control that keeps all five disconnected logical-model modules unreachable from recursively scanned `.ts` and `.mjs` runtime sources. It fails on forbidden literal references, syntax errors and new unreviewed computed module references. The only reviewed computed exception is the existing writing-checker import. The wrapper refuses with exit 2 before checks when the exact-pinned TypeScript compiler is unavailable.
- `extension/route.ts` — the route planner (`route-*`): the SAFETY CORE of action-level routing, i.e. the argument check the code numbers guard 0 (effort vocabulary), guard 1 (router-ON list membership), guards 2–4 (per-model ladder validity, evidence gap and API-rejected level on both router states when profile data is available), guard 7 (the failover list-membership carve-out, which still checks profile-declared provider-rejected effort), the explicit dispatch-field check, and the retained all-dropped fault. The planner derives no model or effort default. Guards 5 and 6 were retired with context-size substitution and long-context billing notices. It was extracted from `threads.ts` into a pure module for exactly this harness, because a guard that silently stops guarding still "works": the dispatch runs and an episode is written;
...
- `extension/model-profiles.ts` — structural invariants of the shipped table (identifier and alias resolution, ladder versus measured and gap coverage, tier range and freezing). Router and doctrine checks cover registry base rates and configured order. Other research data remain a review concern;
...
- **Re-run it after any change to a covered module or source path.** Covered production modules are:
  ...
  - `extension/logical-model-recovery.ts`, `extension/logical-model-adapters.ts` and `verification/logical-model-import-check.ts`, which also require the focused logical-model policy and recovery tests.
  ...
  The workflow contract checks also read `docs/track-workflow.md`, `docs/blast-radius.md`, `docs/review-rules.md`, `docs/user-notes.md`, `docs/pr-publishing.md`, `docs/design-principles.md`, and `README.md`. Re-run them after a change to the policy that any of those documents supplies. The context-budget checks separately read `docs/context-budget.md`. Re-run them after a change to dispatch inputs, reviewer private-input rules, the stuck-fix episode exception, or the measured reference and specialist charter blocks.
