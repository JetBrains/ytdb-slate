...
### Pure-resolver checks

`verification/run-resolver-checks.sh` tests pure production pipelines with
fabricated in-memory inputs. It starts no Pi session and reads no real state.
The closed roster includes these current areas:

- worker-extension selection, load units, barriers, collisions, and doctrine
  rendering
- logical-model definitions, configuration resolution, runtime activation,
  exact reviewed imports, fixed effort, recovery planning, and effective output
- logical doctrine trust, sanitation, numbering, exact portable render sizes,
  runtime rejection boundaries, and whole-doctrine growth controls
- state snapshot sanitation, episode headers, base-model reduction, writing
  policy, writing reminders, worker reminders, and workflow contracts

The logical import check parses TypeScript and JavaScript module references.
It requires the exact reviewed active edge roster for recognized static imports,
exports, import-equals declarations, dynamic imports, and direct `require` calls.
It does not recognize arbitrary loader forms. Its brand-cast check rejects direct
and `unknown` or `any` two-step named `as` assertions in every scanned extension
source except `extension/logical-model-runtime.ts`. In `extension/threads.ts`, it
also rejects angle-bracket assertions to either protected brand and `as never`.
The syntax-only scan does not resolve renamed imports, local or transitive type
aliases, or type flow. A passing typecheck cannot replace this bounded source
scan, and the scan does not make the brands tamper-proof.

Run it with `bash verification/run-resolver-checks.sh --repo . --strict`. Exit 0
means every check passed. Exit 1 means a check failed or disappeared. Exit 2
means the wrapper refused to start. Re-run it after a change to any covered
production module, doctrine source, workflow contract, import edge, or published
budget figure. `verification/README.md` records the fixture basis and check
families.

Doctrine size uses portable characters. The check removes each installed
...
26,300 whole-doctrine ceiling. A valid logical policy with an enlarged supported
...
`worker-extensions.ts`, `state.ts`, `base-model.ts`, `episodes.ts`, writing and
reminder modules, `worker.ts`, and `threads.ts`. Re-run the ladder as well after
changes to shared model-switch, handoff, worker-session, or saved-default
behavior.
...
`npm test` runs `verification/run-tests.sh`: it runs `verification/link-peers.sh`, then partitions the discovered `node:test` roster into a native TypeScript process and a separate mandatory real-Pi host process, then runs the patch-coverage gate in `verification/coverage-gate.mjs`. The host process contains only `test/recovery-ownership-host.test.ts`. It has an independent temporary directory and Jiti cache, and it writes separate host LCOV. It receives no patch-coverage credit. The native LCOV remains the only gate input. The runner requires a disjoint, exhaustive roster, and either process can fail the run. It needs `node`, `git` and a pi. The linker resolves `PI_BIN`, then the exact-pinned `node_modules/.bin/pi`, then `PATH`, and deliberately refuses to start without one.
...
- **The harness adds no test-only dependency and no publish payload.** It uses Node built-ins, shell, `git`, the SDK bundled with pi, and the exact-pinned `typescript` devDependency. TypeScript has four direct uses: `npm run typecheck`, the coverage gate's executable-line classifier, package-content analysis, and the logical-model import-syntax check. `test/`, `verification/` and `tsconfig.json` stay outside the `files` whitelist (§ Packaging rules), so no part of this reaches a consumer install.
...
**The permitted kinds include `extension/**/*.mjs`, and that is deliberate.** The shipped writing checker is dependency-free plain JavaScript. It runs as a command and inside a hot turn hook without a transpiler. The pack policy must permit this runtime command. `verification/package-content-check.mjs` enumerates runtime commands independently. It proves that each has one exported path and packed file.
...
The `release-state` branch keeps an additive durable lifecycle. It binds the original request, reviewed notes and path set, exact merge and its own parent, complete executable check roster, inspected archive, upload execution, registry bytes, clean install proof, promotion intent and observations, Git tag, and GitHub release. Version alone is never release identity. Each preparation dispatch adds a unique authorization generation to the request identity. A rerun of that workflow run keeps the generation. Preparation scans the additive release-state history at the commit bound to its lease and refuses any identity with a terminal record. A failed history read or parse refuses preparation. A still-prepared own identity can resume. Every later stage and operator action must carry the launch identity and reject a different current owner before an effect or state write. Compare-and-swap pushes fence the workflow's own writers.

One release permits one live package upload. The publisher binds the upload to one GitHub run and run-attempt pair and publishes under the internal `slate-candidate` npm distribution tag. Unknown upload blocks retry, abandonment, promotion, and final records. Retirement of an unknown upload requires a completed sealed run attempt, a completed non-success upload job, 60 minutes since job completion, and a fresh full npm package document from an empty cache with earlier versions and publication times. The version must be absent from both `versions` and `time`. A not-found response, read failure or malformed document refuses retirement without a state write. The terminal state keeps the archive and upload execution, marks the upload `observed-absent`, and frees the lane for a new identity. The gate records an observation, not proof that npm never accepted a request. A wrong verdict can leave only an unpromoted version under `slate-candidate` with the sealed archive. `latest` never moves without proof, and a later same-version upload fails and records unknown or mismatch. Recovery from `upload-unknown` requests failed jobs and their dependents from the original release run. The sealed upload authority rejects the upload rerun. Proof-gated dependents may later record registry and installation proof, promote, and create final records. Installation proof is read-only and may be retried under a later run attempt. Its success or failure result binds the producing execution to the exact request identity, version, release commit, parent, and verified registry record. Recovery from `published` state also requests failed jobs and their dependents. The recovery job itself grants no upload or promotion authority. Other stage artifacts bind the request identity, release commit, and owning stage execution. A late result cannot replace the outcome of a newer owner. A confirmed byte mismatch is terminal and records no uploader attribution that the evidence cannot prove.
...
The workflow observes `latest` before and after promotion. The `not-attempted` route applies only when the control code copied to `release-state` at preparation recognizes it. In Bash, run `set -o pipefail` and `git fetch origin release-state`. Run `git show origin/release-state:verification/release-job.mjs | awk '/result:"not-attempted"/ { n++ } END { print n+0 }'` and `git show origin/release-state:verification/release-control.mjs | awk '/"not-attempted"/ { n++ } END { print n+0 }'`. A positive count in both files means the route applies. Zero in either count means it does not apply. Stop if a command fails. Inspect `executePromotion` to confirm that the missing-token and failed first `latest`-read branches return that result. Inspect the control module for result support. Under that control code, either stop produces `not-attempted` with a fixed cause and no npm write. The promoter saves a result and emits one fixed GitHub error annotation. The recorder resolves it to `proved` and clears the promoter. Recovery retries `refused` and `not-attempted` only while `latest` equals the saved expected value, through a newly authorized protected promoter. Do not rerun a workflow run whose checked-out `.github/workflows/release.yml` lacks the `not-attempted` retry condition in `recover` for a release whose stored control code supports it. Check the workflow file at that run's commit, not just current `main`. A rerun of the original promoter cannot retry. If a promotion result artifact exists but its recorder failed, rerun only that recorder first while the original intent owns state. Do not choose `recover` first: while state remains `promotion-unknown`, it reads `latest`, records `superseded` when `latest` remains at the expected value, and leaves only `close`. A failed promotion recorder can download the original fixed-name result artifact on a later run attempt when the original intent still owns state. It refuses observed conflicts and does not roll back another observed selection. npm tag writes are unconditional, so this does not protect against an independent npm writer inside the read-and-write window. Nobody may change `latest` manually while release automation is active.

Ordinary abandonment covers only an unmerged request with no upload. A merged prepared request retires only after a successful GitHub read shows exactly one merged pull request into `main` from this repository and its identity-bound branch. Failed or malformed reads and zero or multiple matches refuse the write. Prepared retirement uses current control code and keeps null claim fields. A claimed no-upload authorization also uses explicit retirement. Claim requires the durable owner to match the identified launch identity and refuses terminal or replaced owners. Do not retire while a preparation or claim run that started before pull request #439 merged is still active. Do not rerun a preparation or claim run that started before pull request #439 merged. GitHub permits reruns for 30 days. An observed-absent unknown upload may also retire under the evidence gate above. Preparation refuses a version listed in npm `versions` or `time`, and refuses package-not-found or failed registry reads. A corrected request may reuse the same unused version and may omit manifest files when `main` already names that version. A proved release with a resolved but impossible promotion may close without promotion. A registry-verified published release with durable installation-failure evidence may also close without promotion. The attempt-keyed `install-failure-<attempt>` artifact feeds `record-install-failure`, which commits validated evidence under `install-failures/`. If that recorder fails, failed-job recovery must rerun the failed installation job and recorder together. A recorder-only rerun requests an artifact under a new attempt number that no installation job produced. This route requires resolved upload and promotion outcomes and no active publisher or promoter. Retirement, mismatch, and close preserve evidence, revoke the exact identity, create no success final records, and free the lane. A published close consumes the version and rejects late installation results.

The release request changes only `package.json`, `package-lock.json`, and its three files under `release/requests/<version>/`. A same-version correction can change a subset of the three request files, including `request.json`. The request records the fixed permitted set. A version-changing request must change both manifests and `request.json`. Those files remain outside the package `files` whitelist. The workflow moves no dogfood pin. Existing SDK pins change only during a targeted pi release.

`verification/release-checks.sh` is the executable release roster. Keep every listed command, order, strict flag, exit status, and verdict check. A refusal, failure, unsupported result, absent verdict, or `NOT RUN` stops before publication. A coverage `WARN` advances only when the changed paths are a subset of the reviewed permitted set, include `request.json`, and include both manifests for a version-changing request. The exact parent and identity enter evidence.
