...
`verification/run-resolver-checks.sh` is the automated net for the repo's PURE pipelines. It loads and exercises six modules against fabricated in-memory registries, fabricated profile tables, fabricated events and an injected clock — no pi session, no real state — in ~1 second:
