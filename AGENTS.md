...
Every child starts through one `env -i` launcher with validated throwaway `HOME`, `TMPDIR` and `PI_CODING_AGENT_DIR`, a guarded tool path, offline mode and an exact environment allowlist. The launcher requires a positive start marker. Corpus evidence, every agent root recorded durably by the guarded pi shim, and an untouched throwaway-HOME fallback agent are the redirect proof. The shim records roots from actual pi starts, independently of rung call sites. A focused rung with no pi child records a positive not-applicable session PASS. The real settings content hash is a nonfatal concurrency note, not a verdict.
