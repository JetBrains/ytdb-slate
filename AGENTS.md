...
- **Automated verification exists.** Tier-1 CI runs four checks on every pull request, on every merge-queue candidate, on a push to `main`, and on a manual dispatch. Each check is the same command that you run locally, and the four checks together take about five seconds (§ Tier-1 CI). Four tasks stay manual:
  ...
  4. **Run the verification ladder by hand** (tier 2, § Verification ladder below). Tier-1 CI leaves it out on purpose.
...
**A pipeline hides the exit code of a process, and that trap probably produced the wrong claim.** `$?` after a pipeline gives the status of the **last** command, and not the status of the piped process. A broken extension shows the difference. A direct run of pi exits `1`. The same run as `pi … 2>&1 | head -1` gives a `$?` of `0`, while `${PIPESTATUS[0]}` for that pipeline is still `1`, and `set -o pipefail` makes the pipeline exit `1`. Read a real exit code in one of three ways: use no pipe, read `${PIPESTATUS[0]}`, or run `set -o pipefail` first. Every script in `verification/` runs `set -o pipefail`.
  ...
  **Run the typecheck after ANY TypeScript change.** That trigger is wide on purpose, and the two deep nets below use a narrow trigger instead. The run takes about 2 seconds, and it checks every file that it covers in one pass. Spend no time on a decision about the type relevance of your edit.

  Two deep nets cover the mechanisms that fail in silence: the verification ladder and the pure-resolver checks below. The typecheck is NOT one of them. It sees shapes, it never sees behaviour, and every mechanism in those sections fails with correct types. Run the net that covers your change, and run both nets when your change touches something that they share.
...
`verification/run-ladder.sh` is the deepest regression net in this repo. It is not the only net: § Tier-1 CI lists four more, and the pure-resolver checks below are a fifth. It is the only net that touches the global model-default machinery. It covers:
...
This repo pins each release from end to end. It dogfoods the exact version that it publishes, because `.pi/settings.json` pins `npm:ytdb-slate@<version>`. A consumer moves to a new version only through a change of a pin. The order of the steps below therefore matters.

1. Bump the four SDK pins in `devDependencies` when you target a newer pi release, and refresh `package-lock.json`. Do this **in step with the pi release that you target**. The typecheck sees upstream SDK drift only after such a bump, so a breaking SDK change stays invisible here until then. This is a recorded **accepted risk** and not an oversight, because no tool tells us about a change upstream.
2. Bump `version` in `package.json`.
3. Run tier 1 locally before you publish. Run all four checks (§ Tier-1 CI), and not the typecheck alone.
4. `npm publish`.
