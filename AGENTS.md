...
Every child starts through one `env -i` launcher with validated throwaway `HOME`, `TMPDIR` and `PI_CODING_AGENT_DIR`, a guarded tool path, offline mode and an exact environment allowlist. The launcher requires a positive start marker. Selected-agent session and corpus evidence plus an untouched throwaway-HOME fallback agent are the redirect proof. The real settings content hash is a nonfatal concurrency note, not a verdict.

Concurrent pi sessions are permitted only when they cannot write this checkout. A fatal before-and-after repository fingerprint covers HEAD, the index, tracked files, relevant untracked files and their metadata. A same-checkout writer remains prohibited. Same-user edit-and-restore is the documented residual boundary because no portable userspace fingerprint can observe it after the fact. The inherited `PI_CODING_AGENT_DIR` guard remains fatal because the ladder must choose every child redirect itself.
