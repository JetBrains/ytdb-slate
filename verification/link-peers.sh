...
if [ -n "${PI_BIN:-}" ]; then
  ...
  pi_bin="$PI_BIN"
  printf 'link-peers: NOTE using PI_BIN override %s\n' "$pi_bin"
elif [ -x "$repo/node_modules/.bin/pi" ]; then
  pi_bin="$repo/node_modules/.bin/pi"
elif command -v pi >/dev/null 2>&1; then
  pi_bin="$(command -v pi)"
  printf 'link-peers: NOTE no checkout-local pi; using PATH copy %s\n' "$pi_bin"
else
  fail "pi is not installed; run npm ci --ignore-scripts, set PI_BIN, or put pi on PATH"
fi
