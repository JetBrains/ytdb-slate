...
| `doctrine-budget` | a **guard**, not a timeless fact, measured on an install-invariant figure. The check removes each absolute docs-directory occurrence and keeps the filename. It separately bounds the routing rule, writing rule, worker rule, model rows and representative combinations. Its stable maximum uses all nine profiles, draft PRs, writing, two capped worker units and four capped tools. That fixture measures **7,358 of 7,400** portable characters. The **42-character reserve is 0.6% of the 7,400 bound**. The bound rose from 7,200 because the doctrine now names the permission contract, enabling setting, and prompt-cache rewrite. The binding checked combination is writing plus routing at **5,992 of 6,000**, with eight characters left. The capped worker rule measures **1,347 of 1,600** characters. An **8,122-character** positive control adds one capped tool and three maximum-growth model rows. It exceeds the bound by 722 and proves the guard can fail. These are verification budgets, not runtime limits. Arbitrary extension rosters can exceed them. |
...
| writing plus router | 6 | **5,992** | — | 6,000 |
| writing plus extensions | 5 | **3,662** | — | 6,000 |
| writing plus router and extensions | 6 | **6,247** | — | 6,300 |
| maximal doctrine, 9 profiles plus draft PRs, writing, and capped worker fixture | 7 | **7,358** | 100 | 7,400 |
| positive control, one extra capped tool plus three maximum-growth model rows | 7 | **8,122** | — | must exceed 7,400 |

Writing plus routing keeps the smallest reserve at eight portable characters.
The maximal doctrine keeps 42 characters, or 0.6 percent of its bound. The old
7,200 bound could not preserve the required permission, setting, and rewrite
semantics. The largest model-row growth is 184 characters. The extra capped tool
line is 212 characters. The positive control exceeds the bound by 722 characters.
