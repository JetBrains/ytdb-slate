...
| `doctrine-budget` | a **guard**, not a timeless fact, measured on an install-invariant figure. The check removes each absolute docs-directory occurrence and keeps the filename. It separately bounds the routing rule, writing rule, worker rule, model rows and representative combinations. Its stable maximum uses all nine profiles, draft PRs, writing, two capped worker units and four capped tools. The draft-enabled fixture measures **7,662 of 8,200** portable characters. A separate draft-disabled pin measures **7,643**. Every upper bound must exceed its measurement by at least five percent. Required character raises round up to the next hundred. Writing plus routing measures **6,296 of 6,700**. The capped worker rule measures **1,347 of 1,600** characters. An **8,610-character** positive control adds one capped tool and four maximum-growth model rows. It exceeds the maximal bound by 410 and proves the guard can fail. These are verification budgets, not runtime limits. Arbitrary extension rosters can exceed them. |
| `doctrine-budget-follow-up` | the trusted maximal fixture with `workflow.followUpIssues: true` measures **7,740 of 8,200** portable characters. It shares the stable maximal basis and bound, keeps 460 characters of reserve, and proves the conditional sentence is charged to the budget. |
...
| router-off doctrine | 4 | **2,641** | 46 | — |
| router-on doctrine | 5 | **5,226** | 70 | 6,500 |
| writing-only doctrine | 5 | **3,711** | 68 | 5,600 |
| writing plus router | 6 | **6,296** | 92 | 6,700 |
| writing plus extensions | 5 | **3,966** | 74 | 6,000 |
| writing plus router and extensions | 6 | **6,551** | 98 | 6,900 |
| maximal doctrine with draft PRs enabled | 7 | **7,662** | 102 | 8,200 |
| maximal doctrine with draft PRs disabled | 6 | **7,643** | 102 | 8,200 |
| maximal doctrine with follow-up issues enabled | 7 | **7,740** | 103 | 8,200 |
| positive control, one extra capped tool plus four maximum-growth model rows | 7 | **8,610** | 107 | must exceed 8,200 |
...
reserve policy requires it. Writing plus routing keeps 404 portable characters.
Writing plus router and extensions keeps 349. The enabled maximal doctrine keeps
538 characters. The disabled one keeps 557. The follow-up-issues maximum keeps
460. The largest model-row growth is 184 characters. The capped tool-line growth
is 212 characters. The positive control exceeds the maximal bound by 410.
