...
| `doctrine-budget` | a **guard**, not a timeless fact, measured on an install-invariant figure. The check removes each absolute docs-directory occurrence and keeps the filename. It separately bounds the routing rule, writing rule, worker rule, model rows and representative combinations. Its stable maximum uses all nine profiles, draft PRs, writing, two capped worker units and four capped tools. That fixture measures **7,331 of 7,400** portable characters. The **69-character reserve is 0.9% of the 7,400 bound**. The bound rose from 7,200 because the doctrine now names all three permission states, the enabling setting, and the meaning of a prompt-cache rewrite. The capped worker rule measures **1,347 of 1,600** characters. An **8,095-character** positive control adds one capped tool and three maximum-growth model rows. It exceeds the bound by 695 and proves the guard can fail. These are verification budgets, not runtime limits. Arbitrary extension rosters can exceed them. |
...
| maximal doctrine, 9 profiles plus draft PRs, writing, and capped worker fixture | 7 | **7,331** | 99 | 7,400 |
| positive control, one extra capped tool plus three maximum-growth model rows | 7 | **8,095** | — | must exceed 7,400 |

The maximal doctrine keeps 69 portable characters of reserve. That is 0.9
percent of the 7,400-character bound. The old 7,200 bound could not preserve the
required permission, setting, and rewrite semantics. The largest model-row
growth is 184 characters. The extra capped tool line is 212 characters. The
positive control exceeds the bound by 695 characters.
