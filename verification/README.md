...
| `doctrine-budget` | a **guard**, not a timeless fact, measured on an install-invariant figure. The check removes each absolute docs-directory occurrence and keeps the filename. It separately pins every path count, rule size, line count, fixed fabricated six-model basis, all-nine basis, worker rule, model-row increment and tool-line increment. It also pins the untrusted doctrine at 2,709 portable characters, 43 lines and three embedded paths. It derives the maintained fixture tables, their listed arithmetic, and every current prose statement of an enforced doctrine limit in this document and `docs/context-budget.md`. It compares each maintained statement with the value used by enforcement and requires each statement once. The roster does not interpret future prose. A maintainer must add each new limit statement to the roster. Other measurements and numeric prose still need manual review. It reads no project config. The draft-enabled maximum is **8,569 of 9,100** portable characters. The draft-disabled pin is **8,550**. Writing plus routing is **7,203 of 7,600**. All tails are **7,458 of 7,900**. The capped worker rule is **1,347 of 1,600**. A **9,885-character** positive control exceeds the maximal bound by 785. These are verification budgets, not runtime limits. |
| `doctrine-budget-deferred` | the trusted maximal fixture with `workflow.followUpIssues: true` is **8,643 of 9,100** portable characters and 107 lines and keeps 457 characters of reserve. |
...
| `contract-focus-table-sync` | both marked focus tables occur once, remain equal, and match the complete fixed eight-row table. The project README names the same eight areas exactly once |
| `contract-risk-definitions` | all eight risk definitions match exact text, use no area numbers, and keep the marked test-quality block. A negated counterfactual must differ |
...
| `contract-review-charters` | Reviewer I has the exact three added clauses and runs at every grade. The current non-local logic defect charter, `NL` prefix and production-cap class are exact. The charter assigns no duty to an area that the current roster does not provide. Test, prose, and licensing charters stay additional. The composite test role stays separate, prose and licensing stay split, and retired charters and prefixes stay absent |
...
| trusted router-off doctrine | 4 | **4,618** | 72 | — |
| trusted router-on doctrine | 5 | **7,203** | 96 | 7,600 |
| fabricated fixture mirroring current dogfood config, pinned extensions, and pi-registry windows | 6 | **7,425** | 102 | — |
| writing and design doctrine | 4 | **4,618** | 72 | 5,600 |
| writing plus router | 5 | **7,203** | 96 | 7,600 |
| writing plus extensions | 4 | **4,873** | 78 | 6,000 |
| writing plus router and extensions | 5 | **7,458** | 102 | 7,900 |
| maximal doctrine with draft PRs enabled | 6 | **8,569** | 106 | 9,100 |
| maximal doctrine with draft PRs disabled | 5 | **8,550** | 106 | 9,100 |
| maximal doctrine with deferred-issue prompt enabled | 6 | **8,643** | 107 | 9,100 |
| positive control, one extra capped tool plus six maximum-growth model rows | 6 | **9,885** | 113 | must exceed 9,100 |
...
`7,203 × 1.05 = 7,563.15`. Ceiling gives 7,564. The 7,600 bound is larger.

All tails use `7,458 × 1.05 = 7,830.9`. Ceiling gives 7,831. The 7,900 bound
is larger.

The largest maximal fixture uses `8,643 × 1.05 = 9,075.15`.
Ceiling gives 9,076. The 9,100 bound is larger. The enabled, disabled, and
deferred-issue maximal reserves are 531, 550, and 457.

The largest model-row growth is 184 characters. The capped tool-line growth is
212 characters. The positive control exceeds the maximal bound by 785.
