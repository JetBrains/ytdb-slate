...
| `doctrine-budget` | a **guard**, not a timeless fact, measured on an install-invariant figure. The check removes each absolute docs-directory occurrence and keeps the filename. It separately pins every path count, rule size, line count, fixed fabricated six-model basis, all-nine basis, worker rule, model-row increment and tool-line increment. It also pins the untrusted doctrine at 2,717 portable characters, 44 lines and four embedded paths. It reads no project config. The draft-enabled maximum is **24,447 of 25,800** portable characters. The draft-disabled pin is **24,428**. Writing plus routing is **23,081 of 24,300**. All tails are **23,336 of 24,600**. The capped worker rule is **1,347 of 1,600**. A **34,595-character** positive control exceeds the maximal bound by 8,795. These are verification budgets, not runtime limits. |
| `doctrine-budget-deferred` | the trusted maximal fixture with `workflow.followUpIssues: true` is **24,521 of 25,800** portable characters and 183 lines and keeps 1,279 characters of reserve. |
...
| `contract-focus-gates` | effective focus states, conditional design phases, per-track design entry, per-reversal design-review permission, the exact clean-review ending, approved area removal, marker availability, complete-set late-area routing, conditional Reviewer I composition, no-area model choice, separate user-fix verification, and the exact prospective P11 unit stay explicit. Bounded mutations reject a one-total allowance, a mandatory round, the wrong review stage, an alternate clean ending, grade conditions, widened unconditional exceptions, universal evidence, and current-artifact weakening. Explicit design-review markers fail closed when missing, duplicated, or malformed. Benign edits immediately before and after the marked unit, and at end of file, remain accepted. Acceptance policy itself belongs to `contract-acceptance-units` and `contract-acceptance-mutations` |
| `contract-publishing-migration` | three independent bounded units cover draft creation, no-design description inputs and final presentation, later-design handling, and both migration directions. Each unit resolves exactly once and matches an independent expectation. Mutations restore grade conditions, reverse timing or migration, lose the conditional log input, or recreate a later draft. Missing and duplicate boundaries fail closed. Benign edits outside the units remain accepted |
...
| `contract-dispatch-context` | four complete owned policy units cover the focused implementation-context obligation, research-log lifecycle, reviewer-input contract, and stuck-fix policy. Each unit resolves exactly once and equals an independent expectation. Counterfactuals remove current-context inputs or restore a mandatory whole-log read, weaken a private-source ban, reverse implementation-review inputs, add direct-read permission, add gate or reviewer episodes, and add episode screening or filtering. Each counterfactual must fail. Missing and duplicated units fail closed. Benign changes outside the units remain accepted. The focused reference and four charter regions keep their exact UTF-8 byte measurements and fail-closed boundaries |
...
The resolver suite prints the live expected-identifier count and result-line count in its roster summary. That output remains the definition of record.
...
| trusted router-off doctrine | 5 | **4,771** | 74 | — |
| trusted router-on doctrine | 6 | **23,081** | 172 | 24,300 |
| fabricated fixture mirroring current dogfood config, pinned extensions, and pi-registry windows | 7 | **21,108** | 179 | — |
| writing and design doctrine | 5 | **4,771** | 74 | 5,600 |
| writing plus router | 6 | **23,081** | 172 | 24,300 |
| writing plus extensions | 5 | **5,026** | 80 | 6,000 |
| writing plus router and extensions | 6 | **23,336** | 178 | 24,600 |
| maximal doctrine with draft PRs enabled | 7 | **24,447** | 182 | 25,800 |
| maximal doctrine with draft PRs disabled | 6 | **24,428** | 182 | 25,800 |
| maximal doctrine with deferred-issue prompt enabled | 7 | **24,521** | 183 | 25,800 |
| positive control, one extra capped tool plus six maximum-growth model rows | 7 | **34,595** | 189 | must exceed 25,800 |
...
`23,081 × 1.05 = 24,235.05`. Ceiling gives 24,236. The 24,300 bound is larger.

All tails use `23,336 × 1.05 = 24,502.80`. Ceiling gives 24,503. The 24,600 bound
is larger.

The largest maximal fixture uses `24,521 × 1.05 = 25,747.05`.
Ceiling gives 25,748. The 25,800 bound is larger. The enabled, disabled, and
deferred-issue maximal reserves are 1,353, 1,372, and 1,279.

The largest model-row growth is 1,656 characters. The capped tool-line growth
is 212 characters. The positive control exceeds the maximal bound by 8,795.
