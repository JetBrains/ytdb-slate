...
| `doctrine-planner-default` | parses the advertised default from every real doctrine row, then compares it with every planner seed path for every shipped profile. It covers fresh bases, base re-seeds, stored-effort refresh, model-local action derivation and context substitution. A marker and the planner cannot drift independently. |
| `doctrine-budget` | a **guard**, not a timeless fact, measured on an install-invariant figure. The check removes each absolute docs-directory occurrence and keeps the filename. It separately bounds the routing rule, writing rule, worker rule, model rows and representative combinations. Its stable maximum uses all nine profiles, draft PRs, writing, two capped worker units and four capped tools. That fixture measures **6,909 of 7,200** portable characters. The **291-character reserve is 4.0% of the 7,200 bound**. The capped worker rule measures **1,347 of 1,600** characters. A **7,673-character** positive control adds one capped tool and three maximum-growth model rows. It exceeds the bound by 473 and proves the guard can fail. These are verification budgets, not runtime limits. Arbitrary extension rosters can exceed them. |
...
| maximal doctrine, 9 profiles plus draft PRs, writing, and capped worker fixture | 6 | **6,909** | 93 | 7,200 |
| positive control, one extra capped tool plus three maximum-growth model rows | 6 | **7,673** | — | must exceed 7,200 |

The maximal doctrine keeps 291 portable characters of reserve. That is 4.0
...
exceeds the bound by 473 characters.
...
| `router-recommendation-validation` | candidate construction rejects a non-string value, an empty string, an off-ladder level and a provider-rejected level before any becomes `recommendedEffort` |
...
| `route-lowest-effort` | an effort seed uses the validated recommendation first, then the **lowest measured, non-provider-rejected** fallback in pi vocabulary order. The fallback is never an evidence gap, and it is `undefined` when no measured level exists, the model is unlisted, the router is off, or the resolution is junk |
| `route-recommended-fresh` / `route-recommended-null` / `route-recommended-stored` / `route-recommended-substitute` / `route-recommended-failover` / `route-recommended-invalid` | pins recommendation precedence, null fallback, stored-state priority, model-local substitution, failover isolation, warning deduplication and the no-fallback message |
| `route-recommended-revalidate` | a forged provider-rejected recommendation is caught again at seed time, while malformed rejection metadata falls back without throwing |
...
- **"inheriting a level across models (BG14)" no longer killed anything.** Not
  ...
  case (stored `medium`, legal on both; the target has no recommendation and its
  measured fallback is `low`), and the mutation kills again. A fix making a mutation harmless is good news;
