...
try {
	...
	if (!table) {
		...
		await section("doctrine-budget", async () => {
			...
			// 2026-08-07: 6800 → 6900 because the user chose Option 3, restoring the enforcement trigger, class confirmation, presentation order, and two previously unstated gates.
			const ALL_TAILS_BOUND = 6900;
			...
			checkAll(
				...
				[
					...
					["the router-off doctrine is the measured 2641 portable chars", portable(off).length === 2641, { portable: portable(off).length, lines: off.split("\n").length }],
					["...and the whole router-on doctrine is the measured 5226 portable chars and stays under 6500 with five percent reserve", portable(on).length === 5226 && portable(on).length <= 6500 && hasDoctrineReserve(portable(on).length, 6500), { portable: portable(on).length, raw: on.length, lines: on.split("\n").length }],
					["writing-only doctrine is the measured 3711 portable chars and stays under 5600 with five percent reserve", portable(writingOn).length === 3711 && portable(writingOn).length <= 5600 && hasDoctrineReserve(portable(writingOn).length, 5600), { portable: portable(writingOn).length, lines: writingOn.split("\n").length }],
					[`writing plus router is the measured 6296 portable chars and stays under ${WRITING_ROUTER_BOUND} with five percent reserve`, portable(writingRouterOn).length === 6296 && portable(writingRouterOn).length <= WRITING_ROUTER_BOUND && hasDoctrineReserve(portable(writingRouterOn).length, WRITING_ROUTER_BOUND), { portable: portable(writingRouterOn).length, lines: writingRouterOn.split("\n").length }],
					["writing plus extensions is the measured 3966 portable chars and stays under 6000 with five percent reserve", portable(writingExtensionsOn).length === 3966 && portable(writingExtensionsOn).length <= 6000 && hasDoctrineReserve(portable(writingExtensionsOn).length, 6000), { portable: portable(writingExtensionsOn).length, lines: writingExtensionsOn.split("\n").length }],
					[`all three tail features are the measured 6551 portable chars and stay under ${ALL_TAILS_BOUND} with five percent reserve`, portable(writingAllOn).length === 6551 && portable(writingAllOn).length <= ALL_TAILS_BOUND && hasDoctrineReserve(portable(writingAllOn).length, ALL_TAILS_BOUND), { portable: portable(writingAllOn).length, lines: writingAllOn.split("\n").length }],
					// Update exact measurements with production wording in the same commit.
					[`the maximum all-feature fixture is the measured 7662 portable chars and stays within ${MAXIMAL_BOUND} with five percent reserve`, maximalPortable === 7662 && maximalPortable <= MAXIMAL_BOUND && hasDoctrineReserve(maximalPortable, MAXIMAL_BOUND), { portable: maximalPortable, raw: maximal.length, lines: maximal.split("\n").length, profiles: realCandidates.length, units: MAX_EXT.units.length, tools: MAX_EXT.units.reduce((n, unit) => n + unit.tools.length, 0) }],
					[`the draft-PR-disabled maximum fixture is pinned independently at 7643 portable chars and shares the ${MAXIMAL_BOUND} maximum bound`, maximalNoDraftPortable === 7643 && maximalNoDraftPortable <= MAXIMAL_BOUND && hasDoctrineReserve(maximalNoDraftPortable, MAXIMAL_BOUND), { portable: maximalNoDraftPortable, raw: maximalNoDraft.length, lines: maximalNoDraft.split("\n").length, profiles: realCandidates.length, units: MAX_EXT.units.length, tools: MAX_EXT.units.reduce((n, unit) => n + unit.tools.length, 0) }],
					...
					[`the positive control is the measured 8610 portable chars and exceeds ${MAXIMAL_BOUND} by the larger growth unit`, overBudgetPortable === 8610 && overBudgetPortable > MAXIMAL_BOUND && overBudgetPortable - MAXIMAL_BOUND >= Math.max(maxModelIncrement.growth, maxToolIncrement), { portable: overBudgetPortable, lines: overBudget.split("\n").length, bound: MAXIMAL_BOUND, growthBeyondBound: overBudgetPortable - MAXIMAL_BOUND, maxModelIncrement, maxToolIncrement }],
