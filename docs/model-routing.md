...
not implemented; they are tracked as a deferred issue.
...
- **the effort level** — the `effort` argument (one of pi's thinking
  levels), else a level DERIVED for the model that actually runs
  (see [Effort levels](#effort-levels)).

Neither is sticky. An explicit `model` routes THAT action and does
not become the thread's default; the next action that omits it goes
back to the thread's base, and the next action that omits `effort`
goes back to the level the worker session opened on. A thread's base
model and base effort are separate state, visible in the `threads`
listing, and are never set from an action's own route.
...
3. and if that model has no measured level at all, nothing is set:
   the worker session's own opening level applies.
...
| --- | --- |
| effort vocabulary (0) | a level pi has no name for — and a non-STRING `effort` argument, which read as "omitted" would quietly run the action at a different level than the caller asked for |
...
| API-rejected level | a guaranteed provider failure. Checked before the ladder, because such a level IS on the model's pi ladder |
...
- **In the `threads` listing:** `base=<model>@<level>?` — what a
  ...
  the LEVEL as provisional: it is a stored default, re-checked on
  every dispatch. Then `last=<model>@<level>` — what the thread's
...
not a bug.** With a six-model list, resolution emits roughly ten
lines — eleven exactly, in the stock case broken down below. They
are not repeated: every warning fires at most once per session, and
the resolution is frozen after the first consultation. The batch
therefore lands when the router is first consulted — in practice
the session's first dispatch — rather than at process start; the
config-shape warnings above land earlier, at session start.

The breakdown for a six-model list on a stock pi install:
...
Two obligations that the shipped profile data states are **doctrine
obligations, not code-enforced guards**. Nothing in the dispatch
path implements them, and a warning must not be read as an
interlock:
...
- **The compliance refusal.** `anthropic/claude-fable-5` has no
  ...
  compliance rules.
- **The review/verification carve-out on unevidenced levels.** An
  action that ran at a level with no capability measurement, and
  output from a model whose profile carries a verification gate at
  the high effort levels, are to be treated as unevidenced and
  verified before they are relied on. The code contributes exactly
  two things: the ⚠ notice at dispatch and the `(unmeasured level)`
  marker in the episode header. Nothing checks that a verification
  happened, and nothing blocks the dispatch —
  `allowUnmeasuredEffort: false` is a blanket refusal of unevidenced
  levels for the whole project, not this carve-out.
...
`extension/model-profiles.ts` that ships with the package: nine
models — the six routed ones plus three cheap-tier models that are
profiled but out of scope for routing, so that naming one gets you
data instead of a spurious "no profile" warning (all three are
marked non-preferred). There is no network access and no runtime
refresh; the table changes only when the package is republished.
...
wrong research data.** The resolver-check suite asserts STRUCTURE
only for this table — ids and aliases resolve, ladders are
...
- **The router bounds the price of an action, not a budget.** There
  is no spend cap and no cross-action cost target; per-action cost
  attribution is tracked as a separate deferred issue.
