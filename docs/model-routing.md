...
- **In the orchestrator's own system prompt, every turn:** the
  ...
  introduces. The current fixed fabricated six-model fixture grows Slate's
  block by 2,030 portable characters and 21 lines. The final router-off and
  router-on totals are maintained with the full doctrine fixtures in
  `docs/context-budget.md` and `verification/README.md`. The fabricated fixture
  is independent of this repository's model configuration. Those are PORTABLE
  characters — the  doctrine with each occurrence of the installed `docs/` directory
