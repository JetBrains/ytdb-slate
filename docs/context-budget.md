...
| --- | --- | --- | --- | --- | --- | --- |
| off | — | off | absent | 4 | 4,762 | 73 |
| off | — | off | set | 4 | 4,762 | 73 |
| off | — | on | absent | 5 | 4,781 | 73 |
| off | — | on | set | 5 | 4,781 | 73 |
| on | fixed six-model fixture | off | absent | 5 | 18,428 | 167 |
| on | fixed six-model fixture | off | set | 5 | 18,428 | 167 |
| on | fixed six-model fixture | on | absent | 6 | 18,447 | 167 |
| on | fixed six-model fixture | on | set | 6 | 18,447 | 167 |
| on | all 9 shipped | off | absent | 5 | 23,072 | 171 |
| on | all 9 shipped | off | set | 5 | 23,072 | 171 |
| on | all 9 shipped | on | absent | 6 | 23,091 | 171 |
| on | all 9 shipped | on | set | 6 | 23,091 | 171 |
...
| --- | ---: | --- | ---: | ---: | ---: | ---: |
| fixture mirroring current `.pi/slate.json`: draft PRs + writing, pi-registry windows | 7 resolved | pinned-package 2 units / 4 tools | 6 | 21,099 | 178 | ≈5,275 |
| stable maximal fixture | 9 | synthetic 2 units / 4 tools, every rendered field at its cap | 6 | 24,438 | 181 | ≈6,110 |
| deferred-issue maximal fixture | 9 | synthetic 2 units / 4 tools, every rendered field at its cap | 6 | 24,512 | 182 | ≈6,128 |
...
| trusted router-on doctrine | 23,072 | 24,300 | 1,228 |
| writing and design doctrine | 4,762 | 5,600 | 838 |
| writing plus router | 23,072 | 24,300 | 1,228 |
| writing plus extensions | 5,017 | 6,000 | 983 |
| writing plus router and extensions | 23,327 | 24,500 | 1,173 |
| maximal doctrine, draft PRs enabled | 24,438 | 25,800 | 1,362 |
| maximal doctrine, draft PRs disabled | 24,419 | 25,800 | 1,381 |
| maximal doctrine, deferred-issue prompt enabled | 24,512 | 25,800 | 1,288 |
...
router-on fixture requires `23,072 × 1.05 = 24,225.6`. The existing
24,300 bound is larger.

The all-tail fixture requires `23,327 × 1.05 = 24,493.35`. The trusted
router-on reserve is 1,228 characters. The all-tail reserve is 1,173
characters. The 24,500 bound is larger.

The deferred-issue fixture requires `24,512 × 1.05 = 25,737.6`. The shared
25,800 bound keeps the required reserve for every maximal fixture.

The positive control adds one capped tool and six copies of the largest
measured model row. It measures 34,586 portable characters. It exceeds the
25,800-character maximal bound by 8,786. That margin remains larger than the
...
the nearest whole token. The shipped-rule table ranges from about 1,191 tokens
to about 5,769 tokens. The current dogfood basis is about 5,275 tokens. The
stable representative maximum is about 6,110 tokens. The deferred-issue
maximum is about 6,128 tokens, or 2.39 percent of the default budget.
