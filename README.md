...
With the router off, an omitted `model` plans for the thread's pre-router pin. A thread without a pin gets no plan target. An omitted `effort` resolves no level. The worker session then returns to what it opened on. [`docs/model-routing.md`](docs/model-routing.md) defines the full contract and its exceptions.

Slate routes only models with shipped benchmark profiles. It drops unserved or unprofiled entries with a warning.
