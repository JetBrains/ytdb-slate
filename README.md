...
| `maxConcurrent` | number | `4` | Maximum number of worker actions running concurrently (must be ≥ 1). Excess dispatches wait in a queue; actions on the same thread always run in dispatch order. Default rationale: shipped `docs/design-principles.md`. |
