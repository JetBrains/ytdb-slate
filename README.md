...
With this repository's six-model list and pi's stock registry, Slate hides ten notes. Slate shows one discoverability line instead. A local registry override can change that count. The line names the hidden warning count and the `router.showWarnings` setting. The notes may inform an explicit model and effort choice. They do not select or reroute an action.
