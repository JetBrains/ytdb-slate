...
| `preserveGlobalModelDefault` | boolean | `true` | Restore pi's global model defaults (`defaultProvider`, `defaultModel`, `defaultThinkingLevel`) after a Slate-initiated model switch — orchestrator failover and handoff adoption — reverting only the keys that switch changed, and only to the values it produced. `false` leaves the switched values persisted globally. Best-effort, with limits (an abandoned restore, third-party writes in the window): [`docs/model-failover.md`](docs/model-failover.md). |
